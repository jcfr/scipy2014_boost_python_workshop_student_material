...
public:
    ...
    Move nextMove(const std::vector<Round>& history,
                  unsigned char my_pos) const  {
        bp::list hist;
        BOOST_FOREACH(Round r, history) {
            hist.append(r);
        }
        return bp::extract<Move>(
            player_.attr("next_move")(hist, my_pos));
    }
...
BOOST_PYTHON_MODULE(rps)
{
    ...
    bp::class_<Player, boost::noncopyable>("Player", bp::no_init)
        .add_property("name", &Random::name, &Random::setName)
        ;
    ...
    bp::class_<PyPlayer, bp::bases<Player> >(
        "PyPlayer",
        boost::python::init<const std::string&, bp::object>())
        ;
    ...
    bp::class_<Round>("Round",
                      bp::init<Move, Move>())
        .def_readonly("p1", &Round::p1)
        .def_readonly("p2", &Round::p2)
        ;
